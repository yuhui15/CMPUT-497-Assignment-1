"""
Word Sense Disambiguation (WSD) Evaluation Script
-------------------------------------------------

This script evaluates system-generated sense annotations against a gold standard
using accuracy as the metric. It can optionally report results broken down
by part of speech (POS).

Usage (command line):
---------------------
    python evaluate_wsd.py --answer_file <answers> --gold_file <golds> [--by_pos]

Arguments:
    --answer_file : Path to system output file
    --gold_file   : Path to gold standard file
    --by_pos      : If set, reports results separately by POS

Example:
--------
    python evaluate_wsd.py --answer_file system.key.txt --gold_file gold.key.txt
"""
