...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Extracts alignments based on different embeddings", epilog="example: python3 SimAlign_properly_modified.py path/to/L1/tokens path/to/L2/tokens [options]")
	parser.add_argument("L1_path", type=str, help="Path to pre-tokenized L1 file (one sentence per line, space-separated tokens)")
	parser.add_argument("L2_path", type=str, help="Path to pre-tokenized L2 file (same format as L1)")
	...
	parser.add_argument("--token-type", type=str, choices=["bpe", "word"], default="word", help="Use 'word' for pre-tokenized input (recommended)")
	...
	# MODIFIED: Read pre-tokenized files directly
	for path in original_paths:
		corpus = [l.strip() for l in codecs.open(path, 'r', 'utf-8').readlines()]
		# Clean control characters but preserve spaces between tokens
		corpus = [regex.sub("\\p{C}+", "", l).strip() for l in corpus]
		original_corpora.append(corpus[:max_sent_id])

	# MODIFIED: Handle pre-tokenized input properly
	# For word-level alignment with pre-tokenized input, we need to:
	# 1. Split by spaces to get your pre-defined tokens
	# 2. Still use BERT tokenizer for subword handling (necessary for embeddings)
	# 3. Map subwords back to your original word tokens
	
	words_tokens = []
	for sent_id in range(len(original_corpora[0])):
		# Split by spaces - these are YOUR pre-defined tokens from Excel
		l1_words = original_corpora[0][sent_id].split()
		l2_words = original_corpora[1][sent_id].split()
		
		# CRITICAL: We still need BERT's subword tokenization for getting embeddings
		# But we'll average them back to your word-level tokens later
		l1_tokens = [embed_loader.tokenizer.tokenize(word) for word in l1_words]
		l2_tokens = [embed_loader.tokenizer.tokenize(word) for word in l2_words]
		words_tokens.append([l1_tokens, l2_tokens])

	# Create BPE token lists and word-to-BPE mappings
	# This mapping allows us to average subword embeddings back to word level
	sentences_bpe_lists = []
	sentences_b2w_map = []
	for sent_id in range(len(words_tokens)):
		# Flatten: convert [[bpe1, bpe2], [bpe3]] to [bpe1, bpe2, bpe3]
		sent_pair = [[bpe for w in sent for bpe in w] for sent in words_tokens[sent_id]]
		# Map each BPE to its original word index
	...
	out_f = {ext: open('{}_{}.txt'.format(args.output, ext), 'w') for ext in matching_methods}
	if args.log:
	...
	if args.null_align < 1.0:
		...
		for sent_id in range(len(original_corpora[0])):
			...
			if convert_to_words:
				...
				# This maps back to YOUR original tokens from Excel
	...
	for batch_id, batch_sentences in enumerate(tqdm(data_loader)):
		...
		for in_batch_id, sent_id in enumerate(batch_sentences[0].numpy()):
			...
			if not convert_to_words:
				...
				# CRITICAL: This averages subword embeddings back to YOUR word tokens
				vectors = SentenceAligner.average_embeds_over_words(vectors, words_tokens[sent_id])
				sim = SentenceAligner.get_similarity(vectors[0], vectors[1])
			...
			# When convert_to_words=True (our case), vectors are already at word level
			# So i and j directly correspond to YOUR original token indices
					...
					for ext in matching_methods:
						if all_mats[ext][i, j] > 0:
							...
							if args.token_type == "bpe":
								...
								# word mode: i and j ARE your original token indices
								b2w_aligns[ext].add('{}-{}'.format(i, j))

			for ext in out_f:
				if convert_to_words:
					if not args.add_probs:
						# Output: indices correspond to YOUR original tokens
